(function (namu) {
	...
	function onPageLoad() {
		...
		if (pathname.indexOf("/w/") === 0) {
			...
			if (!namu.userSettings['footnote_type']) {
				...
				if (namu.isMobile()) {
					...
					$fnSelect.children("option[value='popover']").attr("selected", true);
					$fnSelect.val('popover');
				}
				$fnSelect.change();
			}
		}
		...
		if(namu.userSettings['hide_heading_content']) {
			...
			$(".wiki-heading.wiki-heading-folded").addClass('wiki-heading-content-folded');
		}
	...
	function postApplyLayoutCustom() {
		...
		if (namu.userSettings['hide_namulive_card'] == true) {
	...
	function applyUserCustom() {
		...
		if (pathname.indexOf("/w/") === 0) {
			...
			if (namu.userSettings['footnote_type'] == "popover") {
				...
				// -- do nothing...
			} else if (!namu.userSettings['footnote_type']) {
				if(namu.isMobile()) {
					applyPopupFootnote();
				} else {
					applyPopoverFootnote();
	...
	function getDocumentTitle() {
		...
		if (pathname.indexOf("/w/") == 0) {
			...
			return pathname.substr(9);
		} else if(pathname.indexOf("/edit/") == 0) {
			return pathname.substr(6);
		} else if(pathname.indexOf("/xref/") == 0) {
			return pathname.substr(6);
		} else if(pathname.indexOf("/discuss/") == 0) {
			return pathname.substr(9);
		} else {
			return null;
		}
	...
	function refreshLiveCard(isFirst) {
		if (!$("aside.sidebar").is(":visible")) {
			setTimeout(refreshLiveCard.bind(null, isFirst), 5000);
			return;
		}
		if (document.hasFocus && !document.hasFocus() && !isFirst) {
			setTimeout(refreshLiveCard, 10000);
			return;
		}
		...
		$cards.each(function () {
			...
			var $title = $card.children("h5");
			var $link = $card.children("a");
			var title = getDocumentTitle();

			$.ajax({
				url: "//namu.wiki/live-api/bywiki.json" + ( title ? ("?title=" + title) : ""),
				dataType: "json"
			}).done(function (data) {
				if (!data.result) {
					$table.empty();
				...
				$title.text(data.data.slug === 'headline' ? '라이브 헤드라인 최신글' : '라이브 ' + data.data.board);
				$link.attr('href', '//namu.live/b/' + data.data.slug);
				$table.empty();
				[].map.call(data.data.articles, function (article) {
					$table.append(
						$("<a data-npjax='true'>").attr('href', '//namu.live/b/' + data.data.slug + '/' + article.id)
						.append($('<span class="leaf-info">').text((function(name) {
							if (name && 2 <= name.length && name.substr(name.length - 3) === ' 채널') {
								return name.substr(0, name.length - 3);
							}
							return name;
						})(article.board)))
						.append($("<span>").text(article.title))
					);
				});
			}).fail(function () {
				$table.empty();
	...
	function refreshLiveTopbarArea(isFirst) {
		...
		if (!$topbarArea.is(":visible")) {
			setTimeout(refreshLiveTopbarArea.bind(null, isFirst), 5000);
			return;
		}
		if (document.hasFocus && !document.hasFocus() && !isFirst) {
			setTimeout(refreshLiveTopbarArea, 10000);
			return;
		}
		var title = getDocumentTitle();
		...
		$.ajax({
			...
			for (var i in articles) {
				...
				var $unit = $('<div class="treadmill-unit">');
				$unit.append($('<span class="leaf-info">').text(item.board));
				$unit.append($('<a data-npjax="true">').attr('href', '//namu.live/b/' + data.data.slug + '/' + item.id).text(item.title.substr(0,100)));

				$topbarArea.append($unit);
			}

			$topbarArea.startTreadmill({ diection: "up", speed: "slow" });

			//setTimeout(refreshLiveTopbarArea.bind(null, false), random(30000, 60000));
		}).fail(function () {
			$topbarArea.empty();
			$topbarArea.append($('<span class="leaf-info">').text('에러!'));
			$topbarArea.append($('<a data-npjax="true">').text('갱신 실패!'));
			setTimeout(refreshLiveTopbarArea.bind(null, isFirst), 1000);
		});
	}
