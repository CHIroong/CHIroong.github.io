var adWrite={
	...
	,initInsertForm:function(data)
	{
		...
		if(type==="popup" || type==="close" || type==="day_close"){
			...
			if(type==="link"){
				$(".adcil_btns_link option[value="+(data.target||"self")+"]").prop("selected", true);
	...
	,float:function(j)
	{
		...
		//console.log(popupWidth);
			...
			,layer:function(){
				...
				.css({"top":top+"px","left":left+"px","width":popupWidth+"px","height":popupHeight+"px","z-index":"1000" + parents.vars.popup.order}).addClass("layer_box"+(layerScroll=="Y"?" layer_box_fixed":"")+(layerBorder=="Y"?" layer_box_border":""))
				...
				if(parents.vars.popup.view=="manager"){
					...
					$content.appendTo($("#user-container"));
				}
			}				
		};
			...
			$btn.each(function(i,v) {
				...
				if($thisBtn.is(".popup-days")){
					...
					if(expire=="day"){
						...
						.not(":has(*)").html('<a href="#link">하루 동안 열지않기</a>');
					}
					// 몇일동안 daySubfix
					else if(expire=="days"){
						...
						var html = '<a href="#link">'
									+ (period>1 ? '몇일 동안 열지않기' : '하루 동안 열지않기')
									+'</a>';
